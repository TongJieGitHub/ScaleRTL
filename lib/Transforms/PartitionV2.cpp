...
#define  GEN_PASS_DEF_PARTITIONV2
...
struct StateInfo {
  ...
  size_t partId;
  llvm::SmallDenseSet<size_t> readPartitions;
};
...
struct KaHyPar {
  size_t nNodes;
  llvm::SmallVector<double> nodeWeight;
...
struct PartitionInfo {
  ...
  void buildGraph(OpBuilder & builder, const llvm::DenseMap<Operation*,size_t>& opOrder, llvm::DenseMap<StringRef, StateInfo>& stateInfo) {
    ...
    for(auto op: dup) {
      ...
      if(info && info->dir == Read) {
        ...
        stateInfo[info->name].readPartitions.insert(id);
      }
    }
    ...
    auto def = builder.create<ksim::DefQueueOp>(loc, "partition_padding_" + std::to_string(id), builder.getI8Type(), 256, SmallVector<int64_t>());
    def->setAttr("partId", builder.getI64IntegerAttr(id));
...
struct PartitionV2Pass : public ksim::impl::PartitionV2Base<PartitionV2Pass> {
  using ksim::impl::PartitionV2Base<PartitionV2Pass>::PartitionV2Base;
  void runOnOperation() {
    ...
    //   errs() << *(temp.first) << ", ptr = " << temp.first << ", OperationId = " << temp.second << "\n";
    // }
    // errs() << "\ndebug begin \n" << mffcCnt << "\ndebug end\n";
    ...
    for(auto &[name, info]: stateInfo) {
      ...
      // errs() << "stateName = " << info.name << ", StateId = " << id << "\n"; 
    }
    dep.propagate();
    ...
    for (size_t i = 0; i < dep.propagateSet.size(); ++i) {
      for(auto id : dep.propagateSet[i]) {
        hgp.nodeWeight[id] += 1.0 / dep.propagateSet[i].size();
        // hgp.nodeWeight[id] += 1;
      }
    }
    for(size_t i = 0; i < dep.propagateSet.size(); i++) {
    ...
    for(auto &part: partitions) {
      part.buildGraph(builder, opOrder, stateInfo);
    }
    func.erase();
    sortStates(mod);
    for(auto &[name, info]: stateInfo) {
      auto partId = info.partId;
      if (info.writeOps.size() == 0) {
        builder.setInsertionPoint(info.defOps);
        auto loc = builder.getUnknownLoc();
        auto def_before = builder.create<ksim::DefQueueOp>(loc, info.name.str() + "_padding_before_" + std::to_string(partId), builder.getI8Type(), 256, SmallVector<int64_t>());
        def_before->setAttr("partId", builder.getI64IntegerAttr(partId));
        builder.setInsertionPointAfter(info.defOps);
        loc = builder.getUnknownLoc();
        auto def_after = builder.create<ksim::DefQueueOp>(loc, info.name.str() + "_padding_after_" + std::to_string(partId), builder.getI8Type(), 256, SmallVector<int64_t>());
        def_after->setAttr("partId", builder.getI64IntegerAttr(partId));
      } 
      else if (info.readPartitions.size() > 1) {
        builder.setInsertionPoint(info.defOps);
        auto loc = builder.getUnknownLoc();
        auto def_before = builder.create<ksim::DefQueueOp>(loc, info.name.str() + "_padding_before_" + std::to_string(partId), builder.getI8Type(), 256, SmallVector<int64_t>());
        def_before->setAttr("partId", builder.getI64IntegerAttr(partId));
...
std::unique_ptr<mlir::Pass> ksim::createPartitionV2Pass(PartitionV2Options options) {
  return std::make_unique<PartitionV2Pass>(options);
}
