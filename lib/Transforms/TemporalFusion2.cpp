...
#define GEN_PASS_DEF_TEMPORALFUSION2
...
struct FusionOptions {
  enum RegEnModel {
    RegEnHalf, RegEnMax1, RegEnAsNormalReg
  } regEnModel = RegEnHalf;
  enum QueueModel {
    FootPrint,
    CriticalDump,
    Performance
  } queueModel = Performance;
  enum PIOModel {
    PIOZeroCost,
    PIOFixedCost,
    PIOQueueCost
  } pioModel = PIOFixedCost;
  bool useRandom = true;
  static FusionOptions createFusionOptions() {
    return (FusionOptions) {
      .regEnModel = RegEnAsNormalReg,
      .queueModel = Performance,
      .pioModel = PIOFixedCost,
      .useRandom = true
    };
...
struct QueueModel {
  ...
  QueueModel(const FusionOptions & options): options(options) {}
  int64_t computeQueueCost(int64_t width, int64_t depth) {
    switch(options.queueModel) {
      case FusionOptions::FootPrint: return width * depth;
      case FusionOptions::CriticalDump: return width * std::min(depth, 1l);
      case FusionOptions::Performance: return width * std::min(depth, 1l);
    }
    assert(false);
  }
  int64_t computeQueueGrad(int64_t width, int64_t depth, Random & rnd) {
    static const int64_t mul = 1 << 20;
    auto randval = options.useRandom ? rnd.integer(128) : 0;
    switch(options.queueModel) {
      case FusionOptions::FootPrint: return mul * depth + 1 + randval;
      case FusionOptions::CriticalDump: return mul * width / (1 + depth) + 1 + randval;
      case FusionOptions::Performance: return mul * width / (1 + depth) + 1 + randval;
    }
    assert(false);
  }
};
...
struct GraphEntry {
  ...
  const FusionOptions & options;
  QueueModel qmodel;
  GraphEntry(Operation * op, SeqGraph & g);
  ...
  inline int64_t computeQueueCost(int64_t width, int64_t depth) {return qmodel.computeQueueCost(width, depth);}
  inline int64_t computeQueueGrad(int64_t width, int64_t depth, Random & rnd) {return qmodel.computeQueueGrad(width, depth, rnd);}
};
...
struct GraphValueEntry {
  ...
  const FusionOptions & options;
  QueueModel qmodel;
  GraphValueEntry(Value value, Node source, SeqGraph & g);
  virtual int64_t computeCost(NodeMap<int64_t> &PMap) = 0;
  virtual void updateArcGrad(NodeMap<int64_t> &PMap, ArcMap<int64_t> &costMap, Random &rnd) = 0;
...
GraphEntry::GraphEntry(Operation * op, SeqGraph & g): op(op), port(op), options(g.options), qmodel(g.options) {}
GraphValueEntry::GraphValueEntry(Value value, Node source, SeqGraph & g): value(value), source(source), options(g.options), qmodel(g.options) {}
...
struct RegEntry : public GraphEntry {
  ...
  int64_t computeCost(NodeMap<int64_t> & PMap) final {
    auto depth = std::max(PMap[inNode] + 1 - PMap[outNode], 0l);
    if(info.en) {
      switch(options.regEnModel) {
        case FusionOptions::RegEnHalf: return computeQueueCost(dataWidth, 1) / 2 + computeQueueCost(dataWidth, depth) / 2;
        case FusionOptions::RegEnMax1: return computeQueueCost(dataWidth, std::max(depth, 1l));
        case FusionOptions::RegEnAsNormalReg: return computeQueueCost(dataWidth, depth);
      }
      assert(false);
    }
    else {
  ...
  void updateArcGrad(NodeMap<int64_t> & PMap, ArcMap<int64_t> & costMap, Random & rnd) final {
    ...
    if(info.en) {
      if(options.regEnModel == FusionOptions::RegEnHalf) {
        grad /= 2;
      }
    }
...
struct OutputEntry: public GraphEntry {
  Node node;
  int64_t cost = 0;
  OutputEntry(Operation * op, SeqGraph & g): GraphEntry(op, g), node(g.addNode()) {
    if(options.pioModel == FusionOptions::PIOZeroCost) {
      for(auto &in: port.in) {
        in = g.addNode();
      }
    }
    else if(options.pioModel == FusionOptions::PIOFixedCost) {
      for(auto &in: port.in) {
        in = g.addNode();
      }
      for(auto operand: op->getOperands()) {
        cost += hw::getBitWidth(operand.getType());
      }
    }
    else if(options.pioModel == FusionOptions::PIOQueueCost) {
      port.in.setAll(node);
    }
  }
  int64_t computeCost(NodeMap<int64_t> & PMap) final {return cost;}
  void updateArcGrad(NodeMap<int64_t> & PMap, ArcMap<int64_t> & costMap, Random & rnd) final {}
  void dumpDot(raw_ostream & out, SeqGraph & g) final {
    dumpDotNode(out, g.graph.id(node), op->getName().getStringRef(), g.PMap[node]);
  }
...
struct MemWriteEntry : public GraphEntry {
  ...
  int64_t computeCost(NodeMap<int64_t> & PMap) final {
    auto depth = computeDepth(PMap);
    return computeQueueCost(addrWidth, 1) 
      + computeQueueCost(dataWidth, 1) 
      + computeQueueCost(1, 1);
  }
  void updateArcGrad(NodeMap<int64_t> & PMap, ArcMap<int64_t> & costMap, Random & rnd) final {
    auto depth = computeDepth(PMap);
...
static void rewriteTargets(mlir::Value clockSignal, mlir::Value value, ArrayRef<ValueTargetInfo> targets, ArrayRef<int64_t> delays, OpBuilder & builder, std::optional<StringRef> name=std::nullopt) {
  ...
  auto queueOp = builder.create<ksim::QueueOp>(value.getLoc(), value, clockSignal, SmallVector<int64_t>(delayVec));
...
struct SimpleValueEntry : public GraphValueEntry {
  ...
  void updateArcGrad(NodeMap<int64_t> &PMap, ArcMap<int64_t> &costMap, Random &rnd) override {
    ...
    for(auto info: targets) {
      auto curCost = arcCost;
      if(options.useRandom) {
        curCost += rnd.integer(16);
      }
      costMap[info.arc] = curCost + 1;
    }
  }
  void dumpDot(raw_ostream & out, SeqGraph & g) final {
    dumpTargetEdges(targets, source, out, g, name);
...
struct BlockArgEntry : public SimpleValueEntry {
  ...
  void updateArcGrad(NodeMap<int64_t> &PMap, ArcMap<int64_t> &costMap, Random &rnd) final {
    if(targets.empty()) return;
    if(options.pioModel == FusionOptions::PIOFixedCost || options.pioModel == FusionOptions::PIOZeroCost) {
      for(auto info: targets) {
        costMap[info.arc] = 1;
      }
    }
...
struct ResetValueEntry : public GraphValueEntry {
  ValueTargetVec targets;
  ResetValueEntry(Value value, Node source, SeqGraph & g)
    : GraphValueEntry(value, source, g) {
    targets = connectValueToTarget(value, source, g);
  }
  int64_t computeCost(NodeMap<int64_t> &PMap) final {return 0;}
  void updateArcGrad(NodeMap<int64_t> &PMap, ArcMap<int64_t> &costMap, Random &rnd) final {
    for(auto info: targets) costMap[info.arc] = 1;
  }
  void dumpDot(raw_ostream & out, SeqGraph & g) final {
    dumpTargetEdges(targets, source, out, g, name);
  }
...
static std::unique_ptr<GraphEntry> buildGraphEntry(Operation * op, SeqGraph & graph) {
  return llvm::TypeSwitch<Operation*, std::unique_ptr<GraphEntry>>(op)
    ...
    .template Case<hw::OutputOp>([&](hw::OutputOp op){return std::make_unique<OutputEntry>(op, graph);})
...
static std::unique_ptr<GraphValueEntry> buildGraphValueEntry(Value value, Node valueNode, SeqGraph & graph, bool isReset=false, bool isBlkArg=false) {
  ...
  if(isReset) {
    return std::make_unique<ResetValueEntry>(value, valueNode, graph);
  }
  else if(isBlkArg) {
...
void SeqGraph::build(hw::HWModuleOp op) {
  ...
  auto outputEntry = static_cast<OutputEntry*>(entries[op.getBodyBlock()->getTerminator()].get());
...
struct TemporalFusion2Pass : ksim::impl::TemporalFusion2Base<TemporalFusion2Pass> {
  using ksim::impl::TemporalFusion2Base<TemporalFusion2Pass>::TemporalFusion2Base;
  bool canBeScheduledOn(hw::HWModuleOp op) {return op.isPublic();}
  void runOnOperation() {
    SeqGraph graph;
    graph.options = FusionOptions::createFusionOptions();
...
std::unique_ptr<mlir::Pass> ksim::createTemporalFusion2Pass(TemporalFusion2Options options) {
  return std::make_unique<TemporalFusion2Pass>(options);
}
