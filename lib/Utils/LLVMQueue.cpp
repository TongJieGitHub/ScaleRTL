...
std::unique_ptr<AbstractQueue> ksim::createQueue(mlir::Location loc, mlir::StringRef name, mlir::Type elemType, int64_t depth, bool isPublic, ArrayRef<int64_t> delays, OpBuilder &builder) {
  ...
  if(width <= 4 && depth * width <= 8192) return createQueue(ShiftQueueType, loc, name, elemType, depth, isPublic, builder);
  if(width * depth <= 8192) return createQueue(VecQueueType, loc, name, elemType, depth, isPublic, builder);
