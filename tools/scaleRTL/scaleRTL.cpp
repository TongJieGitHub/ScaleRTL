...
static cl::opt<std::string> outputDriverFileOpenMPName("out-driver-OpenMP", cl::desc("output driver file name"), cl::value_desc("filename"), cl::init(""), cl::cat(mainCategory));
static cl::opt<std::string> outputDriverFileTaskflowAyncName("out-driver-TaskflowAsync", cl::desc("output driver file name"), cl::value_desc("filename"), cl::init(""), cl::cat(mainCategory));
static cl::opt<std::string> outputDriverFileTaskflowCondName("out-driver-TaskflowCond", cl::desc("output driver file name"), cl::value_desc("filename"), cl::init(""), cl::cat(mainCategory));
...
static cl::opt<int> tol("tol", cl::desc("Maximum iteration to solve the programming problem"), cl::init(10), cl::cat(mainCategory));
static cl::opt<int> batchSize("batchSize", cl::desc("Batch Size"), cl::init(2), cl::cat(mainCategory));
...
static LogicalResult processBuffer(
  ...
  if(inputLevel < KSimLow && KSimLow <= outputLevel) {
    ...
    ksim::BatchOptions batchOptions;
    batchOptions.batchSize = batchSize;
    pm.addPass(ksim::createBatchPass(batchOptions));
    pm.addPass(mlir::createInlinerPass());
  }
   if(parallel) {
    // ksim::PartitionOptions options;
    // options.driverFileOpenMP = outputDriverFileOpenMPName;
    // options.driverFileTaskflowAsync = outputDriverFileTaskflowAyncName;
    // options.driverFileTaskflowCond = outputDriverFileTaskflowCondName;
    // options.components = parallel;
    // pm.addPass(ksim::createPartitionPass(options));
    ksim::PartitionV2Options options;
    options.driverFileOpenMP = outputDriverFileOpenMPName;
    options.driverFileTaskflowAsync = outputDriverFileTaskflowAyncName;
    options.driverFileTaskflowCond = outputDriverFileTaskflowCondName;
    options.components = parallel;
    pm.addPass(ksim::createPartitionV2Pass(options));
  }
